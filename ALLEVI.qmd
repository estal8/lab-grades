...
The `{r} students |> "birth_date"` column of the `{r} "students"` data frame is of class `{r} class(students$birth_date)`.
...
```{r}
# id, their group and the average of grades they obtained in each course, then pivot

df_19 <- students_grades_courses |>
  filter(!is.na(grade)) |>
  select(id, group, grade, course) |>
  summarise(mean_grades = mean(grade), .by = c(id, course, group)) |>
  arrange(id, course) |>
  pivot_wider(names_from = course, values_from = mean_grades)
...
```{r}
df_19 |>
  ggplot(aes(x = `Traditional Medicine and Herbalism`, y = `Buddhist Philosophy and Meditation`)) +
...
```{r}
df_19 |>
  summarise(correlation = cor(`Spirituality and Rituals`, `Martial Arts and Self-Defense`), .by = group) |>
  arrange(group) |>
  knitr::kable()
```
...
# grades most correlated
 most_correlated <- correlations |>
   slice_max(abs(correlation)) # group 13
```

```{r}
df_19 |>
  filter(group == "13") |>
  ggplot(aes(x = `Spirituality and Rituals`, y = `Martial Arts and Self-Defense`)) +
  geom_point()
```
...
```{r}
df_23 <- students_grades_courses |>
  filter(!is.na(grade)) |>
  summarise(avg_courses = mean(grade), .by = c(id, group, course, sex)) |>
  summarise(final_grade = mean(avg_courses), .by = c(id, group, sex)) |>
...
```{r}
# final grades between groups

df_23 |>
  summarise(final_grade_groups = mean(final_grade), .by = group) |>
  arrange(final_grade_groups) |>
  slice_head(n = 5) |>
  knitr::kable() 
```
...
condition1 <- students_grades_courses |>
  filter(!is.na(grade)) |>
  summarise(avg_grade = mean(grade), .by = c(id, course)) |>
  summarise(no_failing_course = all(avg_grade >= 5), .by = id)

condition2 <- students_grades_courses |>
  filter(!is.na(grade)) |>
  summarise(avg_course = mean(grade), .by = c(id, course, module)) |>
  summarise(avg_module = mean(avg_course), .by = c(id, module)) |>
  summarise(passing_modules = all(avg_module >= 10), .by = id)

pass <- condition1 |>
  left_join(condition2, by = "id") |>
  mutate(pass = no_failing_course & passing_modules) |>
  select(id, pass)
```
...
pass <- left_join(df_23, pass, by = join_by(id))
```
...
```{r}
pass |>
  filter(pass == TRUE, final_grade >= 10)
```
